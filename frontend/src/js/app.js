...
function loadUserInfo() {
  ...
  if (user) {
    ...
    // RBAC: Role-based sidebar visibility
    // role=0: Employee — faqat asosiy POS sahifalari
    // role=1: Admin — to'liq biznes boshqaruvi
    // role=2: SuperAdmin — Admin panel + hammasi
    // role=3: Client — dashboard ga kirmaydi

    // Admin va SuperAdmin uchun — Marketplace ko'rinadi
    if (user.role >= 1) {
      document.querySelectorAll('.mp-only').forEach(el => el.style.display = '');
    }

    // Admin va SuperAdmin uchun — Egalar sahifalari (xarajatlar, bizneslar, hisobotlar)
    if (user.role >= 1) {
      document.querySelectorAll('.owner-only').forEach(el => el.style.display = '');
    }

    // Faqat SuperAdmin uchun — Admin panel
    if (user.role === 2) {
      document.querySelectorAll('.admin-only').forEach(el => el.style.display = '');
    }

    // Update Sidebar Brand Info (faqat Admin va yuqori)
    if (user.role >= 1) {
      const brandNameEl = document.querySelector('.sidebar-logo .brand-name');
      if (brandNameEl) {
        brandNameEl.textContent = user.brandName || 'SavdoSklad';
      }
      document.title = `${user.brandName || 'SavdoSklad'} — Biznes Boshqaruv Tizimi`;

      if (user.brandImage) {
        const brandLogoEl = document.querySelector('.sidebar-logo .brand-logo-wrapper');
        if (brandLogoEl) {
...
async function renderDashboard() {
  ...
  if (!bid) {
    content.innerHTML = `
      <div class="empty-state">
        ...
        <p>${t("Yuqoridagi ro'yxatdan biznesingizni tanlang yoki yangi biznes yarating.")}</p>
        <br>
        <button class="btn btn-primary" onclick="navigateTo('businesses')">${t("Biznes yaratish")}</button>
      </div>`;
    return;
  }
  ...
  try {
    ...
    const lowStock = productList.filter(p => p.quantity <= 5).length;
    const inventoryLevel = productList.length > 0 ? Math.min(100, Math.round((productList.filter(p => p.quantity > 0).length / productList.length) * 100)) : 0;
    ...
    const currentMonthLabel = `${t(monthNames[thisMonth])}, ${thisYear}`;
    ...
    content.innerHTML = `
      ...
      <div class="card" style="padding:0; overflow:hidden; margin-top:20px;">
         <div class="card-header" style="padding:24px;">
            ...
            <button class="btn btn-primary btn-sm" onclick="openSaleModal()">${t("Yangi sotuv")}</button>
         </div>
  ...
  setTimeout(() => {
    products.forEach((p, i) => {
      ...
      if (ctx) {
        ...
        miniSparklineCharts.push(chart);
      }
    });
  }, 100);
}
  ...
  container.innerHTML = `
    <div class="table-container" style="border:none; box-shadow:none;">
      <table class="premium-table">
        ...
        <tbody>
          ${currentDashboardTransactions.length === 0 ? `<tr><td colspan="5" style="text-align:center; padding:40px; color:var(--text-muted);">${t("Sotuvlar hali yo'q")}</td></tr>` :
