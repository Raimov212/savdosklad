...
func (r *ExpenseRepo) CreateTotalExpense(te *entity.TotalExpense) (int, error) {
	var id int
	err := r.db.QueryRow(
		`INSERT INTO total_expenses ("total", "cash", "card", "description", "businessId", "createdAt")
		VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
		te.Total, te.Cash, te.Card, te.Description, te.BusinessID, time.Now(),
	).Scan(&id)
	return id, err
}
...
func (r *ExpenseRepo) GetTotalExpensesByBusinessID(bid int) ([]entity.TotalExpense, error) {
	rows, err := r.db.Query(
		`SELECT id, "total", "cash", "card", "description", "businessId", "createdAt" 
		FROM total_expenses WHERE "businessId" = $1 ORDER BY id DESC`, bid,
	)
	if err != nil {
		return nil, err
	}
...
func (r *ExpenseRepo) scanTotalExpense(rows *sql.Rows, t *entity.TotalExpense) error {
	return rows.Scan(&t.ID, &t.Total, &t.Cash, &t.Card, &t.Description, &t.BusinessID, &t.CreatedAt)
}
